...
# Ported to TabbyAPI for ExLlamaV3 backend
...
DeltaMessage = ChatCompletionMessage
FunctionCall = Tool  # In TabbyAPI, Tool is the function call type
...
class Glm4MoeModelToolParser(ToolParser):
    ...
    def extract_tool_calls(
        ...
        def _is_string_type(
            tool_name: str, arg_name: str, tools: Optional[list]
        ) -> bool:
            if tools is None:
                return False
            for tool in tools:
    ...
    def extract_tool_calls_streaming(
        ...
        if end_idx != -1:
            ...
            # TabbyAPI uses ChatCompletionMessage with tool_calls field
            # We return the full tool call in the delta
            delta = DeltaMessage(
                content=extracted_tool_calls.content,
                tool_calls=[tool_call],  # Full ToolCall object
            )
            self.current_tool_id += 1
